...
def extract_invoice_data(cleaned_text):
    ...
    bill_no_match = re.search(
        r"(bill|invoice)\s*(no|number)?\s*[:\-]?\s*(\d+)", cleaned_text
    )
    bill_number = safe_value(
        bill_no_match.group(3) if bill_no_match else None, "Bill number", issues
    )
    ...
    invoice_date = safe_value(
        date_match.group(1) if date_match else None, "Invoice date", issues
    )
    ...
    subtotal = safe_value(
        int(subtotal_match.group(1)) if subtotal_match else None, "Subtotal", issues
    )
    ...
    tax_amount = safe_value(
        int(tax_match.group(1)) if tax_match else None, "Tax amount", issues
    )
    ...
    grand_total_match = re.search(
        r"(grand\s*total|total\s*amount\s*payable)\s*(\d+)", cleaned_text
    )
